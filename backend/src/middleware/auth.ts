...
async function checkWeeklyQuotas() {
  try {
    ...
    // Issue infractions without transaction (simpler approach)
    ...
    for (const log of activityLogs) {
      try {
        ...
        console.error(`[QuotaCheck] Error processing user ${log.user_id}:`, err);
      }
    }

    console.log(`[QuotaCheck] Issued ${infractionsIssued} infractions for ${activityLogs.length} users below threshold`);
  } catch (error) {
...
// Graceful shutdown (removed db.close() calls since they're not exposed)
process.on('SIGTERM', () => {
