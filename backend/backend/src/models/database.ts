...
export async function initializeDatabase() {
  try {
    ...
    // Add refresh token columns if missing
    await pool.query(`ALTER TABLE sessions ADD COLUMN IF NOT EXISTS refresh_token TEXT`).catch(() => {
      pool.query(`ALTER TABLE sessions ADD COLUMN refresh_token TEXT`).catch(() => {});
    });
    await pool.query(`ALTER TABLE sessions ADD COLUMN IF NOT EXISTS refresh_expires_at TIMESTAMP`).catch(() => {
      pool.query(`ALTER TABLE sessions ADD COLUMN refresh_expires_at TIMESTAMP`).catch(() => {});
    });
