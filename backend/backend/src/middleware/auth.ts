...
export function generateToken(userId: number, discordId: string, rank: number | null): string {
  ...
  return jwt.sign(payload, jwtSecret, { expiresIn: `${ACCESS_TOKEN_TTL_MINUTES}m`, algorithm: 'HS256' });
}
...
export async function refreshSessionToken(
  ...
  const accessToken = generateToken(userId, discordId, rank);
  const refreshToken = generateRefreshToken();
  const accessExpiresAt = new Date(Date.now() + ACCESS_TOKEN_TTL_MINUTES * 60 * 1000);
  const refreshExpiresAt = new Date(Date.now() + REFRESH_TOKEN_TTL_DAYS * 24 * 60 * 60 * 1000);

  await db.prepare(
    'UPDATE sessions SET token = ?, expires_at = ?, refresh_token = ?, refresh_expires_at = ? WHERE token = ? AND user_id = ?'
  ).run(
    accessToken,
