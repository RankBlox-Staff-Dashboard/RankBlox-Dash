...
const router = Router();
const ACCESS_TOKEN_MAX_AGE = (parseInt(process.env.ACCESS_TOKEN_TTL_MINUTES || '15', 10)) * 60 * 1000;
const REFRESH_TOKEN_MAX_AGE = (parseInt(process.env.REFRESH_TOKEN_TTL_DAYS || '30', 10)) * 24 * 60 * 60 * 1000;
...
router.post('/roblox/verify', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (req.sessionToken) {
      try {
        const rotated = await refreshSessionToken(
          req.user.id,
          req.user.discordId,
          verificationResult.rank,
          req.sessionToken
        ...
        // Also update the cookies with new tokens
        res.cookie('session', rotated.accessToken, { ...cookieOptions(), maxAge: ACCESS_TOKEN_MAX_AGE });
        res.cookie('refresh', rotated.refreshToken, { ...cookieOptions(), maxAge: REFRESH_TOKEN_MAX_AGE });
      } catch (tokenError) {
