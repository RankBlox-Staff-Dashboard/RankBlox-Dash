...
async function queryUser() {
  ...
  if (!dbConfig.user || !dbConfig.password || !dbConfig.database) {
    console.error('Missing database credentials. Please set DB_USER, DB_PASSWORD, and DB_NAME environment variables.');
    console.log('\nTrying to connect with default values...');
    console.log('You may need to set these in backend/.env file:');
    ...
    if (users.length === 0) {
      ...
      for (const user of users) {
        ...
        const [tickets] = await connection.query(
          `SELECT * FROM tickets WHERE claimed_by = ? OR created_by = ? ORDER BY created_at DESC`,
          [user.id, user.id]
        );
